...
def f(x,t):
    ...
    return(np.sin( 4*math.pi*(x/4+1)*t+ x/10) )
...
pca = PCA()
Y_latent = pca.fit_transform(Y) # apply the pca decomposition

# Step 4: Gaussian process regression constuction
GPs = []
for i in range(min(Y_latent.shape[1],5)):  # Construction of the surrogates models
...
Y_pred = pca.inverse_transform(Y_latent_test_m) # The mean
Y_pred_var = Y_latent_test_s @ (pca.components_**2)
...
plt.plot( t[0], f(X_test,t)[i,:],'-',color='green')
plt.fill_between(t[0], Y_pred[i,:] + 1.96 * np.sqrt(Y_pred_var[i,:]),Y_pred[i,:] - 1.96 * np.sqrt(Y_pred_var[i,:]), color='red', alpha=0.5)
plt.plot( t[0], Y_pred[i,:], '--',color='red')
plt.show()

# Affichage des coefficients de la décomposition
for i in range(pca.n_components_):
    plt.plot( t[0], pca.components_[i],label='component: '+str(i))
plt.show()